...
public class ParallelMultiInstanceBehavior extends MultiInstanceActivityBehavior {
    ...
    protected int createInstances(DelegateExecution execution) {
        ...
        // The parent execution must be set to false, so it wouldn't show up in the execution query when using
        // .activityId(something).
        // Do not we cannot nullify the activityId (that would have been a better solution), as it would break boundary
        // event behavior.
